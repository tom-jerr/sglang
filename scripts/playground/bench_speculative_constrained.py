...
Usage:
# single GPU
python3 bench_speculative_constrained.py --model-path meta-llama/Llama-2-7b-chat-hf --speculative-draft-model-path lmsys/sglang-EAGLE-llama2-chat-7B

# multiple GPU
python3 bench_speculative_constrained.py --model-path deepseek-ai/DeepSeek-V3 --speculative-draft-model-path lmsys/DeepSeek-V3-NextN --tp-size 8 --trust-remote-code --batch-size 1 4 8 16 32 --steps 0 1 2 --topk 0 1 2 4 --num_draft_tokens 0 2 4 8
"""
...
# --- 修改点 1: 定义通用的 JSON Schema ---
# 使用包含思考过程和回复的结构，确保生成长度足够进行 Benchmark
...
# --- 修改点 2: 修改 Prompts ---
# 明确指示输出 JSON，避免模型与约束冲突
SYSTEM_PROMPT = "You are a helpful assistant. You must verify your answer step by step. Output your answer in JSON format."
...
# 将 Prompt 加上 System Prompt 或指令，使其更贴合 JSON 生成
...
def send_one_batch(base_url, num_prompts, batch_size, processor, is_multimodal):
    ...
    results = asyncio.run(
        benchmark(
            ...
            extra_request_body=extra_body,  # 注入约束参数
            profile=None,
        )
    )
...
def main(args, server_args):
    ...
    for i in range(args.start, args.end or len(configs)):
        ...
        # 确保启动参数兼容约束解码（部分版本可能需要显式开启语法后端，但通常默认即可）
        # 如果需要测试 XGrammar 等特定后端，可以在这里添加 --grammar-backend xgrammar
        ...
        record = {
            ...
            "mode": "constrained_json", # 标记记录为约束模式
        }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="output_constrained.jsonl")
