...
class EagleDraftWorker(BaseDraftWorker):
    def __init__(
        ...
        print("LZY: eagle worker v2 initialized")
...
class EAGLEWorkerV2(BaseSpecWorker):
    ...
    def _init_grammar_pinned_buffer(self, num_draft_tokens: int):
        """
        Lazy init pinned CPU buffers for grammar data copy.
        Now just init for max batch size.
        """
    ...
    def _async_copy_grammar_data_to_cpu(
        self,
        ...
        Returns:
            Tuple of (retrieve_next_token_cpu, retrieve_next_sibling_cpu,
                      draft_tokens_cpu, copy_grammar_event)
        """
        bs, num_draft_tokens = verify_input.retrive_next_token.shape
        self._init_grammar_pinned_buffer(num_draft_tokens)
        ...
        copy_grammar_event = torch.cuda.Event()
        copy_grammar_event.record()
    ...
    def _process_last_batch_grammar_data(self, last_batch_and_result):
        """
        ...
        Args:
            last_batch_and_result: Tuple of (last_batch, last_result) from the previous batch
        """
        last_batch, last_result = last_batch_and_result
    ...
    def verify(self, batch: ModelWorkerBatch):
        ...
        copy_grammar_event = None
        if batch.has_grammar:
            # async D2H copy with pin memory
            (
                retrieve_next_token_cpu,
                retrieve_next_sibling_cpu,
                draft_tokens_cpu,
                copy_grammar_event,
            ) = self._async_copy_grammar_data_to_cpu(verify_input)
        ...
        # process last batch grammar data
        if batch.last_batch_and_result is not None:
            self._process_last_batch_grammar_data(batch.last_batch_and_result)

        # Generate vocab mask for constrained decoding
        vocab_mask = None
        if batch.has_grammar:
            # Wait for async D2H copy to complete before using CPU tensors
            copy_grammar_event.synchronize()
